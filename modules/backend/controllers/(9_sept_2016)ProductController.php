...
class ProductController extends Controller
{
    ...
    public function export($model=''){
      require_once  str_replace('/',DIRECTORY_SEPARATOR,Yii::getAlias('@app/components/phpExcel/Classes/PHPExcel.php'));
        ...
        //   
        $objPHPExcel->getActiveSheet()->setTitle('Deliverable Report')
         ->setCellValue('A1', 'SN#')
         ->setCellValue('B1', 'Description_Sage')   
         ->setCellValue('C1', 'Item Code')
         ->setCellValue('D1', 'Description_Web')
         ->setCellValue('E1', 'SE_Directory')
        ...
        if(!empty($model))
        {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('C'.$row,ucwords($value['item_code']));
            $objPHPExcel->getActiveSheet()->setCellValue('D'.$row,ucwords($value['description']));
            $objPHPExcel->getActiveSheet()->setCellValue('E'.$row,$value['se_directory']);
    ...
    public function actionBulkSearchTerms()
    {
     $model= new SearchTermCategory();
     $cat_list  = ArrayHelper::map(ProductCategory::find()->where(['parent_id'=>'0'])->all(), 'id', 'cat_name');
     $product_list  = ArrayHelper::map(Product::find()->all(), 'id', 'id');
     if ($model->load(Yii::$app->request->post())) {
