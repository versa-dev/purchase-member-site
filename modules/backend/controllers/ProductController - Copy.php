...
class ProductController extends Controller
{
    ...
    public function export($model=''){
      require_once  str_replace('/',DIRECTORY_SEPARATOR,Yii::getAlias('@app/components/phpExcel/Classes/PHPExcel.php'));
        ...
        if(!empty($model))
        {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('D'.$row,ucwords($value['rstaus']['status']));
            $objPHPExcel->getActiveSheet()->setCellValue('E'.$row,$value['modificationstatus']['modification_status']);
            $objPHPExcel->getActiveSheet()->setCellValue('F'.$row,$value['product_category']);
            $objPHPExcel->getActiveSheet()->setCellValue('G'.$row,$value['sedirectory']['se_directory']);
    ...
    public function exportsearch($model=''){
      require_once  str_replace('/',DIRECTORY_SEPARATOR,Yii::getAlias('@app/components/phpExcel/Classes/PHPExcel.php'));
        ...
        {
            foreach ($model as $key => $value) {
              $item_code_s=Product::find()->where(['id'=>$value['product_id']])->select('item_code')->one();
            ...
            //$objPHPExcel->getActiveSheet()->setCellValue('A'.$row,$row-1); 
            $objPHPExcel->getActiveSheet()->setCellValue('A'.$row,$item_code_s['item_code']); 
    ...
    {  
        ...
        $data= Product::find()->joinWith(['rstaus','modificationstatus','sedirectory'])->where(['product.id'=>$id])->one();
    ...
    {  
        ...
        $modification_status  = ArrayHelper::map(ModificationStatus::find()->all(), 'id', 'modification_status');
        $se_directory  = ArrayHelper::map(SeDirectory::find()->all(), 'id', 'se_directory');
        if ($model->load(Yii::$app->request->post())) {
          $model->created_date=date('Y-m-d h:i:s');
            ...
            $match=Product::find()->where(['item_code'=>$_POST['Product']['item_code']])->one();
            if(!empty($match)){
               $model->addError('item_code', 'Item code already exists!');
    ...
    {  
        ...
        if ($model->load(Yii::$app->request->post())) { 
          $user_id=$_POST['Product']['user_id'];
          $date=date('Y-m-d h:i:s');
           $model->importfile = UploadedFile::getInstance($model, 'importfile');
           if(empty($_POST['Product']['user_id'])){
              $model->addError('user_id', 'please select Vendor');
           ...
           $i=1; foreach ($line_of_text as $key => $value) {
                 if($i!=1 && !empty($value)){
                  $sql = "INSERT INTO product(user_id,item_code,item_name, description,status,modification_status,se_directory,created_date) VALUES ('$user_id','$value[0]', '$value[1]', '$value[2]', '$value[3]', '$value[4]','$value[5]','$date')";
