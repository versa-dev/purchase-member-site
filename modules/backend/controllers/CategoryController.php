...
class CategoryController extends \yii\web\Controller {
    ...
    public function fetchChildCategories($parent_id='') {
      ...
      public function actionSort($id='') {
            if(!empty($id)){
            $model=\app\models\ProductCategory::find()->where(['parent_id'=>$id])->orderBY('orders ASC')->all();
            }
            else{
              $model=\app\models\ProductCategory::find()->where(['parent_id'=>0])->orderBY('orders ASC')->all();  
            }
                    ...
                    $sql="update  product_category set orders = '".$i."' WHERE id='".$value."'";
    ...
    public function actionCreate() {
        $model = new models\ProductCategory();
    ...
    public function actionAdd($id='') {  
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->published=$_POST['ProductCategory']['published'];
            if($model->save()){
            Yii::$app->session->setFlash('Category Saved');
            return $this->redirect(['manager','id'=>$id]);
            }
    ...
    public function actionDelete($id) {
        ...
        $catu = models\ProductCategory::findOne($id);
        if(count($cats) > 0){
           $cat_str = $id.','.implode(',', $cats);
            $cat_str=rtrim($cat_str, ',');
            Yii::$app->db->createCommand("DELETE FROM product_category WHERE id in (".$cat_str.")")->execute();
    ...
    protected function findModel($id) {
        if (($model = models\ProductCategory::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionStatus($id){
        ...
        $cat = models\ProductCategory::findOne($id);
