...
class ProductController extends Controller
{
    ...
    public function actionBulkSearchTerms()
    {
     ...
     if ($model->load(Yii::$app->request->post())) {
        ...
        if(empty($product_id) || empty($product_id[0])){
        ...
        'product_list'=>$product_list
            ...
            foreach ($product_id as $key => $value) {
               $data=SearchTermCategory::find()->where(['category_id'=>$last_cat,'product_id'=>$value])->one();
              
               $querys=$this->fetchChildCategoriesmain($last_cat);
               $str=rtrim($this->sam, "|"); 
               $str_data=explode("|",$str); 
               $queryses=array_reverse($str_data);
               $this->sam='';
               ...
               if(empty($data)){
                   ...
                   $model->category_id=$last_cat;    
                   $this->getChildrbulk($cat_search,$model,$value);
                   $strss='';
                   if($model->save()){}
               }
            }
