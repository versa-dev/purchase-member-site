...
class ProductController extends Controller
{
    ...
    {  $this->layout='inner';
        ...
        throw new \yii\web\NotFoundHttpException("You can't access this url");
       // Yii::$app->getSession()->setFlash('error',"You can't access this url");
    ...
    public function actionBulkSearchTerms()
    {
     ...
     $model= new SearchTermCategory();
     $cat_list  = ArrayHelper::map(ProductCategory::find()->where(['parent_id'=>'0'])->all(), 'id', 'cat_name');
     $product_list  = ArrayHelper::map(Product::find()->where(['user_id'=>Yii::$app->user->id])->all(), 'id', 'item_code');
     if ($model->load(Yii::$app->request->post())) {
    ...
    public function actionDeletesearchword($id,$product_id)
    { 
        //SearchTermCategory::findOne($id)->delete();
        $data_pro=SearchTermCategory::find()->where(['product_id'=>$product_id])->orderBy('add_tree asc')->all();
            $sam_str=array();
          ...
          return $dada=json_encode($sam_str);
